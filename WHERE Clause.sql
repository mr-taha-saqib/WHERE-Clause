...
select top 1 room.roomno,room.hotelno 
from room
right join hotel on room.hotelno=hotel.hotelno 
order by room.price desc


select 'The hotel whose id is' + hotelno + ' is in ' + city + 'and its name is' + hotelname as res from hotel
select concat('The hotel whose id is' , hotelno , ' is in ' , city , 'and its name is' , hotelname) as res from hotel
...
from booking
where guestno IN(10003,10007) and guestno != 10001